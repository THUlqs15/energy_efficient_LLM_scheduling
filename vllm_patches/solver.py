...
class EnergySchedConfig:
    ...
    def from_env(cls) -> "EnergySchedConfig":
        return cls(
            ...
            solution_mode=int(os.environ.get("VLLM_ENERGY_SOLUTION_MODE", "3")),
            h6_tau_grid_size=int(os.environ.get("VLLM_ENERGY_H6_TAU_GRID", "10")),
...
class Alt1HeuristicSolver:
    """Heuristic solver (H4, H5, and H6 modes).

    Step 1: priority q_n = r_n * min(exp(-s_n), CAP) / l_n
    Step 2: density-greedy fill until L_max or B_max binds
    Step 3: mode-dependent batch/frequency selection, pick argmax utility
    """
    ...
    def solve(
        ...
        if self.cfg.solution_mode == 5:
            return self._solve_h6(reqs, Lmax, Bmax, debug_iter, waiting_capacity)
        ...
        raise ValueError(
            f"Unsupported VLLM_ENERGY_SOLUTION_MODE={self.cfg.solution_mode}; "
            "supported modes are 3 (H4), 4 (H5), and 5 (H6)."
        )
    ...
    def _solve_h5(
        ...
        normalized_slack = s_n_s / deadline_safe_s
        urgency = np.minimum(np.exp(-normalized_slack), cap)
        # H5: slack-only urgency for objective, also capped to prevent overflow
        objective_urgency = np.minimum(np.exp(-normalized_slack), cap)
        ell_safe = np.maximum(tok_arr.astype(np.float64), 1.0)
        ...
        for fi in range(F):
            ...
            # H5 keeps H4 admission, but its utility uses slack-only urgency.
            utility = float((r_n_vec[picked_idx] * objective_urgency[picked_idx]).sum())
            J_f = utility - beta * P_f * ET_s
    ...
    def _solve_h6(
        ...
        q_all = (
            r_n_vec[None, :] - beta * P_f_arr[:, None] * t_nf_all
        ) / ell_safe[None, :]
        orders_all = np.argsort(-q_all, axis=1, kind="stable")

        tau_grid_size = max(1, int(cfg.h6_tau_grid_size))
        tau_min_s = max(float(np.min(s_n_s)), 0.010)
        tau_max_s = float(np.max(s_n_s))
        if tau_max_s >= tau_min_s and tau_grid_size > 1:
            tau_candidates = np.linspace(tau_min_s, tau_max_s, tau_grid_size)
        else:
            tau_candidates = np.asarray([tau_min_s], dtype=np.float64)

        global_best_J = -np.inf
        global_best_f = float(default_f)
        global_best_tau_s = 0.0
        global_best_chosen: List[int] = []
        global_best_et_s = 0.0
        global_best_chunked: dict = {}
        ...
        for fi in range(F):
            ...
            for tau_s in tau_candidates.tolist():
                picked_local: List[int] = []
                chunked_override: dict = {}
                used_tok = 0
                used_waiting = 0
                ...
                for pos, orig_idx in enumerate(order_fi.tolist()):
                    ...
                    cand_has_pf = has_pf or bool(is_pf[orig_idx])
                    cand_has_dc = has_dc or not bool(is_pf[orig_idx])
                    cand_num_p = sum_wp + wp_n + (lat.w_pf if cand_has_pf else 0.0)
                    cand_num_d = sum_wd + wd_n + (lat.w_dec if cand_has_dc else 0.0)
                    cand_et_s = (
                        cand_num_p / f_float + cand_num_d / f_alpha + lat.t_c
                    ) / 1000.0
                    if cand_et_s > tau_s:
                        continue
                ...
                picked_idx = np.asarray(picked_local, dtype=np.int64)
                num_p = sum_wp + (lat.w_pf if has_pf else 0.0)
                num_d = sum_wd + (lat.w_dec if has_dc else 0.0)
                ET_s = (num_p / f_float + num_d / f_alpha + lat.t_c) / 1000.0
                utility = float(r_n_vec[picked_idx].sum())
                J_f_tau = utility - beta * P_f * ET_s

                if J_f_tau > global_best_J:
                    global_best_J = float(J_f_tau)
                    global_best_f = f_float
                    global_best_tau_s = float(tau_s)
                    global_best_chosen = picked_local
                    global_best_et_s = float(ET_s)
                    global_best_chunked = dict(chunked_override)
        ...
        if debug_iter >= 0 and debug_iter % 10 == 0:
            ...
            n_d_ch = len(global_best_chosen) - n_p_ch
            print(
                f"[dbg-h6] iter={debug_iter} all={N}(p={n_p}d={n_d}) "
                f"chosen=B_{len(global_best_chosen)}(p={n_p_ch}d={n_d_ch}) "
                f"f={global_best_f:.0f} tau={global_best_tau_s*1000.0:.2f}ms "
                f"J*={global_best_J:.3f} ET={global_best_et_s*1000.0:.2f}ms "
                f"B_max={B_eff} tau_grid={tau_grid_size}",
                file=sys.stderr, flush=True)
